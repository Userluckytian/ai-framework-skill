...
> **用途：** 为 {{PROJECT_NAME}} 新增项目专属子代理。把下方 `---8<---` 以后的内容复制到
...
mode: subagent
# model: 〔仅当需要固定到特定模型时保留本行并填值（如 {{VISION_MODEL}}），否则整行删除〕
permission:
...
〔你是谁、专管什么、何时被主代理调用。2–3 句。示例开头：你是 XX 审查员，专门负责 {{PROJECT_NAME}} 的 XX。〕
